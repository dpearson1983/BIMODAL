...
__device__ double bispec(int x, float &phi, float4 k); // done
...
// Return the Legendre multipole for l = 0 or l = 2
__device__ float legendre(int l, float mu) {
    if (l == 0) {
        return 1.0;
    } else {
...
__device__ double bispec(int x, float &phi, float4 k) {
...
__global__ void bispec_gauss_32(float4 *ks, double *Bk) {
    ...
    int_grid[tid] = d_wi[threadIdx.x]*d_wi[threadIdx.y]*bispec(threadIdx.x, phi, ks[blockIdx.x]);
    __syncthreads();
...
void model_calc(std::vector<double> &pars, float4 *d_ks, double *d_Bk, std::vector<double> &Bk) {
    ...
    int num_data = Bk.size();
    
    bispec_gauss_32<<<num_data, num_threads>>>(d_ks, d_Bk);
